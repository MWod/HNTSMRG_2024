...
def from_one_hot(tensor : tc.Tensor, num_classes : int):
    output = tc.zeros((tensor.shape[0], 1, tensor.shape[2], tensor.shape[3], tensor.shape[4]), device=tensor.device, dtype=tensor.dtype)
    for i in range(num_classes):
        output[tensor[:, i:i+1, :, :, :] > 0.5] = i
    return output
