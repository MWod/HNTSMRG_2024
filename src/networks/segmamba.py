...
def default_pre_config() -> dict:
    config = {}
    config['in_chans'] = 1
    config['out_chans'] = 3
    config['depths'] = [2, 2, 2, 2]
    config['feat_size'] = [48, 96, 192, 384]
...
def test_1():
    ...
    device = "cuda:0"
    input = torch.randn((1, 3, 128, 128, 128), device=device)
    model = SegMamba(**config).to(device)
    output = model(input)
