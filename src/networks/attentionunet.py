...
def default_pre_config():
    ...
    config['channels'] = (32, 64, 128, 256, 512)
    config['strides'] = (2, 2, 2, 2, 2)
    return config
...
class AttentionUNet(tc.nn.Module):
    def __init__(self, **config):
        super().__init__()   
        self.image_size = config['img_size']
        config.pop('img_size')
        self.model = attentionunet.AttentionUnet(**config)
...
def test_1():
    ...
    ts.summary(model, input_data=[input], device=device, depth=15)  
